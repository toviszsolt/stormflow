  ...
  objTraverse(clone, ({ key, value, parent, isNode }) => {
    ...
    if (isNode) {
      ...
      if (value.default !== undefined) {
        ...
        const typeFinalDefault =
          typeDefault === 'function' ? getType(value.default()) : typeDefault;
...
const applySchema = (source, schema) => {
  ...
  objTraverse(schema, ({ value, path }) => {
    ...
    switch (value.type) {
      ...
      case 'date':
        objPathSet(target, path, timeFromStr(valueSource));
        break;
    }
  });
